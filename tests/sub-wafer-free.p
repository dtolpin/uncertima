# wafer with stronger dependencies and wider grid steps
   
problem wafer {

  space (sitex, sitey, focus, color, angle) /
    0.0*(nz(abs(@sitex)+abs(@sitey)+abs(@focus))+nz(@color)+3*nz(@angle)) {
    sitex = [-1 .. 1];
    sitey = 0;
    focus = [1, 4 .. 31];
    color = [1 .. 5];
  ...
  observe (x, y, px, py) {
    1 => (x, y, px, py) = (1.0, 1.0, 1.0, 1.0)
      / 1E-5; # sufficiently low to prevent premature termination
  }
